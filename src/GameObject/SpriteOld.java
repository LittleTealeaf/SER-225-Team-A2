...
@Deprecated
public class SpriteOld extends Rectangle {
    protected BufferedImage image;
    protected Rectangle bounds;
    protected ImageEffect imageEffect;

    public SpriteOld (BufferedImage image, float scale, ImageEffect imageEffect) {
    ...
    public SpriteOld(BufferedImage image, float x, float y, float scale, ImageEffect imageEffect) {
    ...
    public float getBoundsY2() {
        return getY1() + bounds.getY2();
    }

    public float getScaledBoundsX1() {
        return getX1() + (bounds.getX1() * scale);
    }
    ...
    public float getScaledBoundsY1() {
        return getY1() + (bounds.getY1() * scale);
    }
    ...
    public void setBounds(Rectangle bounds) {
        this.bounds = new Rectangle(bounds.getX1(), bounds.getY1(), bounds.getWidth(), bounds.getHeight(), scale);
    }
    ...
    public void draw(GraphicsHandler graphicsHandler) {
        graphicsHandler.drawImage(image, Math.round(getX1()), Math.round(getY1()), Math.round(getScaledWidth()), Math.round(getScaledHeight()), imageEffect);
    }
    ...
    public String toString() {
        return String.format("Sprite: x=%s y=%s width=%s height=%s bounds=(%s, %s, %s, %s)", getX1(), getY1(), getScaledWidth(), getScaledHeight(),
