...
@Deprecated
public class RectangleOld implements IntersectableRectangleOld {
	...
	public RectangleOld(float x, float y, int width, int height) {
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
	...
	public RectangleOld(float x, float y, int width, int height, float scale) {
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
	...
	public RectangleOld getIntersectRectangle() {
		return new RectangleOld(x, y, getScaledWidth(), getScaledHeight());
	}

	// check if this intersects with another rectangle
	public boolean intersects(IntersectableRectangleOld other) {
		RectangleOld intersectRectangle = getIntersectRectangle();
		RectangleOld otherIntersectRectangle = other.getIntersectRectangle();
	...
	public boolean overlaps(IntersectableRectangleOld other) {
		RectangleOld intersectRectangle = getIntersectRectangle();
		RectangleOld otherIntersectRectangle = other.getIntersectRectangle();
		return Math.round(intersectRectangle.getX1()) <= Math.round(otherIntersectRectangle.getX2()) && Math.round(intersectRectangle.getX2()) >= Math.round(otherIntersectRectangle.getX1()) &&
				Math.round(intersectRectangle.getY1()) <= Math.round(otherIntersectRectangle.getY2()) && Math.round(intersectRectangle.getY2()) >= Math.round(otherIntersectRectangle.getY1());
